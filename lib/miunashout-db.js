...
exports.saveMsgnp = function(data, cb){
	var newMsg = new Chat({msg: xss(data.msg), nickto: xss(data.nickto), uid: data.uid, gid: data.gid, did: data.did, colorsht: xss(data.colorsht), bold: parseInt(data.bold), font: parseInt(data.font), size: parseInt(data.size), avatar: data.avatar, uidto: data.uidto, suid: '0,0', nick: data.nick, edt: '0', edtusr: '0', type: data.type, created: Date.now()});
	newMsg.save(function(err, docs){
